    ...
    update(patch: Row) {
      ...
      const u = {
        ...
        then(onF: (v: unknown) => unknown) {
          for (const r of rows) {
            if (eqs.every(([c, v]) => r[c] === v)) Object.assign(r, patch);
          }
...
Deno.test("dedupe: first call inserts and returns true", async () => {
  ...
  const fresh = await dedupe(100, "fm-1", "t-1", sb);
...
Deno.test("dedupe: second call same (msg,family) returns false", async () => {
  ...
  assertEquals(await dedupe(200, "fm-1", "t-1", sb), true);
  assertEquals(await dedupe(200, "fm-1", "t-1", sb), false);
});

Deno.test("dedupe: same msg different family is independent", async () => {
  const { sb, rows } = mockSupabaseFor("message_log");
  assertEquals(await dedupe(300, "fm-1", "t-1", sb), true);
  assertEquals(await dedupe(300, "fm-2", "t-1", sb), true);
  assertEquals(rows.length, 2);
});

Deno.test("markDone: updates status to done for matching row", async () => {
  const { sb, rows } = mockSupabaseFor("message_log");
  await dedupe(400, "fm-1", "t-1", sb);
  await markDone(400, "fm-1", "t-1", sb);
  assertEquals(rows[0]!.status, "done");
});

Deno.test("markError: writes error and status=error", async () => {
  const { sb, rows } = mockSupabaseFor("message_log");
  await dedupe(500, "fm-1", "t-1", sb);
  await markError(500, "fm-1", "t-1", sb, "boom");
